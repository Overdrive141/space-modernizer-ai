import { roomType, themes, rooms, themeType } from "@/types";
...
const Home: NextPage = () => {
  ...
  const [currentType, setCurrentType] = useState<themeType>("Modern");
  const [currentRoom, setCurrentRoom] = useState<roomType>("Living Room");
  ...
  const generatePhoto = async (fileUrl: string) => {
    ...
    const res = await fetch("/api/generate", {
      ...
      body: JSON.stringify({ imageUrl: fileUrl, currentRoom, currentType }),
    });
    ...
    if (res.status !== 200) {
      ...
      mutate();
      // Rooms can be empty arr or from localStorage
      const rooms =
  ...
  return (
    <div className="flex max-w-6xl mx-auto flex-col items-center justify-center py-2 min-h-screen">
      ...
      <main className="flex flex-1 w-full flex-col items-center justify-center text-center px-4 mt-4 sm:mb-0 mb-8">
        <a
          href="https://twitter.com/FarhanSoftware"
          target="_blank"
          rel="noreferrer"
        ...
        >
          <span className="font-semibold">Generating New Rooms Everyday</span>
        </a>
        <h1 className="mx-auto max-w-4xl font-display text-4xl font-bold tracking-normal text-slate-100 sm:text-6xl mb-5">
          Generate your <span className="text-red-600">dream</span> room
        </h1>
        ...
        <AnimatePresence mode="wait">
          <motion.div className="flex justify-between items-center w-full flex-col mt-4">
            {restoredImage && (
              <div>
                Here&apos;s your remodeled <b>{currentRoom.toLowerCase()}</b> in
                the <b>{currentType.toLowerCase()}</b> theme!{" "}
              </div>
            )}
            <div
              className={`${
              ...
              <>
                <div className="space-y-4 w-full max-w-sm">
                  ...
                  <DropDown
                    item={currentType}
                    // @ts-ignore
                    setItem={(newTheme) => setTheme(newTheme)}
                    itemList={themes}
                  />
                </div>
                <div className="space-y-4 w-full max-w-sm">
                  ...
                  <DropDown
                    item={currentRoom}
                    // @ts-ignore
                    setItem={(newRoom) => setRoom(newRoom)}
                    itemList={rooms}
                  />
                </div>
              ...
              !originalPhoto && (
                <div className="h-[250px] flex flex-col items-center space-y-6 max-w-[670px] -mt-8">
                  <div className="max-w-xl text-gray-300">
                    ...
                    redesign your room today. You will be able to do 5 redesigns
                    per day for free.
                  </div>
                  <button
